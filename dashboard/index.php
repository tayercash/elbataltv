...
<div class="window_popups">
  <div class="mou_popup" id="receipt_preview_popup">
    <div class="mou_popup_container" style="max-width:450px;">
      ...
      <div class="mou_popup_body" style="text-align:center;padding:20px;min-height:150px;display:flex;align-items:center;justify-content:center;flex-direction:column;">
        ...
        <img id="receipt_preview_image" src="" data-i18n-alt="receipt.alt" alt="Receipt" style="max-width:100%;border-radius:12px;display:none;">
...
<header class="app-header">
  ...
  <div class="header-right">
    ...
    <button id="admin-view-btn" class="icon-btn" style="display:none" onclick="sessionStorage.removeItem('view_as_user');window.location.href=BASE_PATH+'/admin/index.php'" title="Admin View">
    ...
    <div class="user-info">
      ...
      <div class="user-avatar" id="header-avatar"><img class="avatar-img" src="" alt="" style="display:none;width:100%;height:100%;border-radius:50%;object-fit:cover;"><span class="avatar-initial">A</span></div>
    </div>
  </div>
</header>
...
$(document).ready(function () {
  ...
  var viewAsUser = window.location.search.indexOf('view_as_user=1') !== -1;
  if (viewAsUser) sessionStorage.setItem('view_as_user', '1');
  viewAsUser = viewAsUser || sessionStorage.getItem('view_as_user') === '1';
  if (viewAsUser) $('#admin-view-btn').show();
  ...
  if (storedRole === 'admin' && !viewAsUser) {
      var p = typeof getPageFromURL === 'function' ? getPageFromURL() : null;
      var qs = p && typeof shortUrl === 'function' ? '?page=' + shortUrl(p) : '';
      window.location.href = BASE_PATH + '/admin/index.php' + qs;
      return;
  }
  roleBadge.attr('data-i18n', 'common.user').text(i18n ? i18n.t('common.user') : 'User');
  ...
  <div class="sidebar-content">
    <div class="sidebar-header">
      <div class="app-logo" style="display:flex;align-items:center;gap:10px;">
        <img src="../assets/img/logo_192.png" alt="" style="width:36px;height:36px;border-radius:8px;box-shadow:0 2px 10px rgba(255,204,0,0.4);border:1.5px solid rgba(255,204,0,0.6);">
        <span style="font-weight:bold;font-size:1.2rem;"><?php echo $siteName; ?></span>
      </div>
      <button id="sidebar-close-btn" class="close-btn" style="background:none;border:none;color:#888;font-size:1.2rem;cursor:pointer;transition:color 0.3s;flex:0;"><i class="fas fa-times"></i></button>
    </div>

    <div class="sidebar-user-section" style="padding:10px 20px;display:flex;align-items:center;gap:15px;background:rgba(255,204,0,0.03);border-bottom:1px solid rgba(255,255,255,0.05);">
      <div class="sidebar-avatar" style="width:45px;height:45px;background:linear-gradient(135deg,#ffcc00,#ff9900);color:#000;font-weight:bold;font-size:1.2rem;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 12px rgba(255,204,0,0.3);overflow:hidden;"><img class="avatar-img" src="" alt="" style="display:none;width:100%;height:100%;object-fit:cover;"><span class="avatar-initial">A</span></div>
      <div style="flex:1;">
        <div class="username" style="font-weight:bold;color:#fff;font-size:1rem;line-height:1.2;"></div>
        <div style="display:flex;align-items:center;gap:8px;margin-top:2px;">
          <span id="sidebar-role-badge" style="font-size:0.75rem;color:#ffcc00;background:rgba(255,204,0,0.1);padding:2px 8px;border-radius:10px;width:fit-content;font-weight:500;"></span>
    ...
    <div class="sidebar-menu" style="flex:1;overflow-y:auto;padding:15px 10px;display:flex;flex-direction:column;">
      <a href="#" class="menu-item active" data-target="assets/pages/user-devices.php"><i class="fas fa-mobile-alt" style="color:#ffcc00;"></i> <span data-i18n="sidebar.devices">الأجهزة المسجلة</span></a>
    </div>

    <div class="sidebar-footer" style="padding:15px 20px;border-top:1px solid rgba(255,255,255,0.05);display:flex;flex-direction:column;gap:8px;">
...
// ==================== PROFILE POPUP ====================
var _profileSelectedAvatar = null; // null = no change, string = avatar selected
...
function loadProfileData() {
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      // Google section
      if (u.has_google) {
        $('#profile-google-section').show();
      } else {
        $('#profile-google-section').hide();
      }
...
$(document).on('click', '#profile-use-google-btn', function() {
  ...
  // Fetch latest profile to get current avatar (which Google keeps updated)
  var uid = parseInt(localStorage.getItem('userId'));
  var tok = localStorage.getItem('userToken') || '';
  var hwid = getHwid();
  $.ajax({
...
$(document).on('click', '#profile-save-btn', function() {
  ...
  var avatar = _profileSelectedAvatar; // null = no change, string = new avatar URL
