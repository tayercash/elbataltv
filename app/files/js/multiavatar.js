function multiavatar(string, sansEnv, ver) {
    ...
    var CryptoJS = CryptoJS || function (h, s) {
        var f = {}
            ...
            , x = t.BufferedBlockAlgorithm = j.extend({
                ...
                _process: function (a) {
                    var c = this._data, d = c.words, b = c.sigBytes, e = this.blockSize, f = b / (4 * e), f;
                    if (a = (f = a ? h.ceil(f) : h.max((0 | f) - this._minBufferSize, 0)) * e,
                        b = h.min(4 * a, b),
                        a) {
                        for (var m = 0; m < a; m += e)
    ...
    for (var part in p.env = hash[0] + "" + hash[1],
        p.env = Math.round(.47 * p.env) + "",
        p.clo = hash[2] + "" + hash[3],
        p.clo = Math.round(.47 * p.clo) + "",
        p.head = hash[4] + "" + hash[5],
        p.head = Math.round(.47 * p.head) + "",
        p.mouth = hash[6] + "" + hash[7],
        p.mouth = Math.round(.47 * p.mouth) + "",
        p.eyes = hash[8] + "" + hash[9],
        p.eyes = Math.round(.47 * p.eyes) + "",
        p.top = hash[10] + "" + hash[11],
        p.top = Math.round(.47 * p.top) + "",
        p) {
        var nr = p[part];
        nr > 31 ? (1 == (nr = nr - 32 + "").length && (nr = "0" + nr),
            p[part] = nr + "C") : nr > 15 ? (1 == (nr = nr - 16 + "").length && (nr = "0" + nr),
                p[part] = nr + "B") : p[part] = 1 == (nr + "").length ? "0" + nr + "A" : nr + "A"
    }
    var final = [];
    for (var part in p) {
        var partV = p[part].substring(0, 2)
            , theme = p[part].substring(2, 3);
        void 0 !== ver && (partV = ver.part,
            theme = ver.theme),
            final[part] = getFinal(part, partV, theme)
    }
    function getFinal(part, partV, theme) {
        var colors = typeof themes[partV] !== "undefined" ? themes[partV][theme][part] : ""
            , svgString = typeof sP[partV] !== "undefined" ? sP[partV][part] : ""
            , regex = /#(.*?);/g
