...
if (isset($_GET["action"]) && $_GET["action"] !== "") {
    ...
    if ($action == "add_hints") {
        ...
        $payment_info = $conn->query("SELECT * From payments WHERE txn_id='$txn_id'");
        if ($payment_info->num_rows > 0) {
            ...
            if ($successfull_url_status == 0) {
                ...
                if ($conn->query("UPDATE mou_thimbles SET num_of_hints='$new_num_of_hints' WHERE acc_id=$user_id") !== false) {
                ...
                if ($conn->query("UPDATE payments SET successfull_url_status=1 WHERE txn_id='$txn_id'") === false) {
