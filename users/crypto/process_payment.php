...
$result = $conn->query("SELECT * FROM `$products_table` WHERE id = " . esc($item_number));
...
// CoinPayments fees
    ...
    if ($result['error'] == 'ok') {
        ...
        $add_new_transaction = mysqli_query($conn, "INSERT INTO $Coin_Payments_table (transaction_id, item_number, payment_status, from_user, payment_gross, currency_code,txn_id  ) VALUES (" . esc($order_id) . ", " . esc($item_number) . " , 0, " . esc($uid) . ", " . esc($amount_to_charge) . ", " . esc($currency) . ", " . esc($coinpayments_txn_id) . ")");
