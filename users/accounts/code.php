...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    header('Access-Control-Allow-Origin: https://www.elbatal-app.com');
            ...
            if ($result_user->num_rows > 0) {
                ...
                if ($code_passed == true) {
                    ...
                    if ($create_new_code_for_user  == true) {
                        $select_rand_code =  $conn->query("SELECT * FROM `$codes_table` WHERE status = 1 AND short_website = 1 ORDER BY RAND() LIMIT 1");
                        ...
                        if ($select_rand_code->num_rows > 0) {
                            ...
                            if ($conn->query("UPDATE `$users_table_name` SET active_code=" . esc($code) . " , code_last_req='$now_utc_date_time' WHERE id=" . esc($user_id)) === TRUE) {
                            ...
                            if ($active_code == $code) {
                                ...
                                if ($select_code_from_codes->num_rows > 0) {
                                    ...
                                    if ($code_status == true) {
                                        ...
                                        if ($conn->query("UPDATE `$users_table_name` SET active_code=" . esc($new_code) . ", code_last_active='$now_utc_date_time' WHERE id=" . esc($user_id)) === TRUE) {
        ...
        if ($page_action == "create_code") {
            if (isset($_GET["num"]) && $_GET["num"] !== "" && isset($_GET["website"]) && $_GET["website"] !== "") {
                ...
                for ($i = 0; $i < $codes_num; $i++) {
                    $code = rand_num(6);
                    $alias = "code_" . rand_string(15);

                    $url_for_code = urlencode("https://www.elbatal-app.com/?code=" . $code);
