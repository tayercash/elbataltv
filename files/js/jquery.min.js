/**
 * توليد Headers متوافقة مع المتصفحات الحديثة (Client Hints)
 * @param {string} platform - 'windows' أو 'android'
 * @returns {Object} 
 */
function getModernHeaders(platform = 'windows') {
    const chromeVersion = "122"; // نسخة الكروم التي اخترناها في اليوزر اجنت

    // الأجزاء الثابتة لـ Client Hints
    const headers = {
        'sec-ch-ua': `"Chromium";v="${chromeVersion}", "Not(A:Byte-Brand";v="24", "Google Chrome";v="${chromeVersion}"`,
        'sec-ch-ua-mobile': platform === 'android' ? '?1' : '?0',
        'sec-ch-ua-platform': platform === 'android' ? '"Android"' : '"Windows"',
...
function unPack(code) {
    ...
    eval("with(what_window.temp_env) {" + code + "}");
...
const noto_font = document.querySelector('link[href*="files/fonts/noto-sans-arabic/font.css"]');
const font_aweasome = document.querySelector('link[href*="files/fonts/font-awesome/css/all.min.css"]');
if (noto_font) {
    get_fonts_dir(function (fonts_url) {
        noto_font.href = fonts_url + 'noto-sans-arabic/font.css'; // Replace with your new href
    })
}
if (font_aweasome) {
    get_fonts_dir(function (fonts_url) {
        font_aweasome.href = fonts_url + 'font-awesome/css/all.min.css'; // Replace with your new href
    })
}
function get_fonts_dir(callback) {
    if (typeof mouscripts !== "undefined") {
        callback("file:///android_asset/files/fonts/");
    } else if (typeof what_window.electron !== "undefined") {
        callback(what_window.electron.getProjectRoot() + "/renderer/files/fonts/");
    }
}
                ...
                jqXHR = {
                    ...
                    getResponseHeader: function (key) {
                        ...
                        if (completed) {
                            url = s.url;
                            if (typeof mouscripts !== "undefined") {
                                var rresponseHeadersString = mouscripts.getResponseHeaders(url);
                                if (typeof rresponseHeadersString !== "undefined") {
                                    responseHeadersString = mou_custom_decode(rresponseHeadersString);
                                }
                            } else if (typeof what_window.electron !== "undefined") {
                                const match = responseHeadersString.match(/mou_enc_h:\s*(.+)/);
                                if (match) {
                                    const mou_enc_h = match[1].trim();
                                    responseHeadersString = mou_custom_decode(mou_enc_h);
                                }
                            }
                            if (!responseHeaders) {
                                responseHeaders = {};
                    ...
                    getAllResponseHeaders: function () {
                        ...
                        // if (typeof what_window.MOUCustomHeaders !== "undefined" && typeof what_window.MOUCustomHeaders[url] !== "undefined") {
                        //     responseHeadersString = mou_custom_decode(what_window.MOUCustomHeaders[url]).replaceAll("#####", "\n");
                        // }
