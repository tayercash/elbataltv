obj = {
    "main_domain": "https://mycima2.wecima.watch/",
    "server_domain": "https://wecima.watch",
    "working_state": true,
    "type": "cats",
    "server_name": "WeCima",
    "server_title": "وي سيما",
                    ...
                    for (i = 0; i < window.now_domains_length; i++) {
                        ...
                        window["get_prop_domain_" + i] = $.ajax({
                            ...
                            success: function (domain_res) {
                                ...
                                if ($(doc).find("head title").text().trim() == "مشاهدة ماى سيما MYCIMA وى سيما WECIMA افلام و مسلسلات اون لاين - وى سيما wecima ماى سيما mycimaمشاهدة ماى سيما MYCIMA وى سيما WECIMA افلام و مسلسلات اون لاين") {
    ...
    start_website: function () {
        ...
        if (getQueryVariable("film_url")) {
            ...
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/أفلام/افلام-عربي-arabic-movies/">أفلام عربية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/أفلام/10-movies-english-افلام-اجنبي/">افلام اجنبي</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/أفلام/افلام-هندي-indian-movies/">افلام هندى</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/أفلام/1-افلام-تركى-turkish-films/">افلام تركى</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/أفلام/1-افلام-وثائقية-documentary-films/">افلام وثائقية</button>`);

            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/13-مسلسلات-عربيه-arabic-series/list/">مسلسلات عربية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/6-series-english-مسلسلات-اجنبي/list/">مسلسلات اجنبي</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/10-series-indian-مسلسلات-هندية/list/">مسلسلات هندية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/1-مسلسلات-اسيوية/list/">مسلسلات اسيوية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/10-مسلسلات-تركية-turkish-series/list/">مسلسلات تركية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/1-مسلسلات-وثائقية-documentary-series/list/">مسلسلات وثائقية</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/مسلسلات-رمضان-2025">مسلسلات رمضان 2025</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/1-مسلسلات-رمضان-2024/list/">مسلسلات رمضان 2024</button>`);
            $("#custom_selectors").append(`<button class="custom_cat" data-url="category/مسلسلات/1-مسلسلات-رمضان-2023-series-ramadan-2023/list/">مسلسلات رمضان 2023</button>`);
    ...
    "load_list_function": function (res, load_type = "first_load") {
        ...
        aflam_posts = [];
        doc = new DOMParser().parseFromString(res, "text/html");
        $(doc).find(".catalog-grid").find(".media-card").each(function () {
            film = {};
            film.url = $(this).find("a[href]").attr("href");
            $(this).find("h2 .year").remove();
            film.title = $(this).find("h2").text().trim();
            if (/^مشاهدة مسلسل(.*)/gm.test(film.title) == true) {
                film.title = /^مشاهدة مسلسل(.*)/gm.exec(film.title)[1].trim();
            }
            if (/^مشاهدة فيلم(.*)/gm.test(film.title) == true) {
            ...
            img_style_string = $(this).find(".media-card__bg").attr("data-lazy-style");
    ...
    load_film_function: function (film) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if ($(doc).find(`.story__content`).text() !== "") {
                    film_data.description = $(doc).find(`.story__content`).text();
                } else {
                    film_data.description = $(doc).find(".post-content").text();
                }

                $(doc).find(`.hero__content .details-list li`).each(function () {
                    tr = {};
                    tr_key = $(this).find("span").text().replace(":", "").trim();
                    tr_val = $(this).find("p").text().trim();
                    if (["مدة العرض", "السنه", "اللغة", "الجودة", "الدولة", "التصنيف", "الإسم الاصلي"].includes(tr_key)) {
                ...
                if (page_type == "film") {
                    if ($(doc).find(".Episodes--Seasons--Episodes").length > 0) {
                        page_type = "muslsal";
                    }
                }
                ...
                if (page_type == "film") {
                    ...
                    if (moasm_num > 0) {
                        ...
                        $(doc).find(".List--Seasons--Episodes a").each(function () {
                            ...
                            mosem_num = /موسم (.*)/gm.exec(mosem_text)[1];
                    ...
                    first_halkat_num = $(doc).find(`.Episodes--Seasons--Episodes a`).length;
                    ...
                    $(doc).find(`.Episodes--Seasons--Episodes a`).each(function () {
                        ...
                        halka["num"] = halka_num;
                        halka["link"] = epo_link;
                        halkat_array.push(halka);
                        if (completed === first_halkat_num) {
                            halkat_data_term = $(doc).find(`.MoreEpisodes--Button`).attr("data-term");
                            all_7alkat_links = mou_aflam_server.get_all_7alkat_links(first_halkat_num, halkat_data_term, halkat_array, function (halkat_links) {
                                $("#eposids_num").text(` ( ${halkat_links.length} ) `);

                                for (i = 0; i < halkat_links.length; i++) {
                                    epo = halkat_links[i];
                                    epo_link = epo["link"];
                                    halka_num = epo["num"];
    ...
    load_7alakat_function: function (this_btn) {
        ...
        $.ajax({
            ...
            success: function (watching_res) {
                ...
                $(watching_doc).find(".watch__server-list li btn[data-url]").each(function () {
                    // watching_source = $(this).attr("data-url").split('\n')[0];
                    watching_source = atob($(this).attr("data-url").replace(/\s+/g, ''));
                    // .replace(/\s+/g, '');
                    ...
                    if (mou_cust_server !== false) {
                        ...
                        $(`<span class="mou_btn watch_btn" data-full_title="${full_title}" data-continue_watch_code="${continue_watch_code}" onclick="what_window.play_dl_mou_cust_server('${mou_cust_server}','${full_title}','${watching_source}',true,this,\`{'Referer':'${encodeURI(link)}'}\`)">${mou_cust_server}</span>`).appendTo(".watch_srces_btns");
...
mou_aflam_servers_array["WeCima"] = obj;